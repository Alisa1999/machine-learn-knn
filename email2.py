...
def test_set_clear():
    ...
    for ech in files:
        ...
        files_1 = fileWalker(ech_move)
        shutil.move(ech, ech_move)  # 把ech移动到ech_move文件夹下
        if ech_initial not in files_1:
...
def email_parser(email_path):
    ...
    punctuations = """,.:<>()*&^%$#@!'";~`[]{}|、\\/~+_-=?"""
    content_list = readtxt(email_path, 'latin-1')
...
def filter(ham_word_pro, spam_word_pro, test_file):
    ...
    for test_path in test_paths:
        # 贝叶斯推断计算与判别实现
        email_spam_prob = 0.0
        spam_prob = 2/3 # 假设P(spam) = 0.5
        ham_prob = 1/3  # P(ham) = 0.5
...
def main():
    ...
    plt.xlabel('次数')
    plt.ylabel('准确率')
    # ham_len = len(fileWalker(ham_file))
    # spam_len = len(fileWalker(spam_file))
    # sum_len =spam_len+spam_len
